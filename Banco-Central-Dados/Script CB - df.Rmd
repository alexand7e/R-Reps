...
IF.Período]]([https://www3.bcb.gov.br/ifPeríodo/#](https://www3.bcb.gov.br/ifPeríodo/#){.uri}).
...
Dados.com %<>% mutate( # 
  `Ativo total` = 
    as.numeric(str_replace_all(`Ativo total`,"\\.", ""))/1000,
  
  `Operações de Crédito` = 
    as.numeric(str_replace_all(`Operações de Crédito`, "\\.", ""))/1000,
...
Dados.sum <- Dados.com %>% 
  ...
  summarise(
            ...
            `Total dos ativos das instituições` = sum(`Ativo total`, na.rm = T), 
            
            `Média de ativos ` = mean(`Operações de Crédito`, na.rm = T),
            `Desvio Padrão ` = sd(`Operações de Crédito`, na.rm = T),            
            `Total das operações de crédito` = sum(`Operações de Crédito`, na.rm = T),
            ) 
```
...
```{r warning=FALSE}
knitr::kable(Dados.sum, format="latex", booktabs = TRUE) %>% 
  kable_styling(latex_options="scale_down") %>%
  add_header_above(c(" ", "Crédito" = 3, "Ativo" = 3)) %>%
